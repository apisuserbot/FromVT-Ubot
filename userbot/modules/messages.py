...
@register(outgoing=True, pattern=r"^\.pg")
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Kenangan,` " + str(count) + " `Kengangan Sudah Tidak Ada`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Anda Telah Menghapus Kenangan,` " + str(count) + "Kenangan Telah Dihapus`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
